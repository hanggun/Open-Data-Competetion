...
import 'package:business_mining/common/util.dart';
...
abstract class ViewStateModel extends BaseViewStateModel {
  ViewStateModel({ViewState viewState}) : super(viewState: viewState);

  // 加载数据
  Future<dynamic> customLoadData();

  initData() async {
    setBusy();
    try {
      dynamic data = await customLoadData();
      if (data == null) {
        setEmpty();
      } else {
        setIdle();
  ...
  query(httpQuery function) async {
    try {
      dynamic result = await function.call();
      if (result == null) {
        setEmpty();
      } else {
        setIdle();
...
typedef httpQuery<E> = dynamic Function();
...
class BaseViewStateModel with ChangeNotifier {
  ...
  BaseViewStateModel({ViewState viewState}) : _viewState = viewState ?? ViewState.busy {
    logger.d('ViewStateModel---constructor--->$runtimeType');
  }
  ...
  void dispose() {
    _disposed = true;
    logger.d('view_state_model dispose -->$runtimeType');
    super.dispose();
  }
}
